...
- **Tests: 912/912 ✅ PASS** (was 907/912). Five shared-memory tests fixed Apr 26 — see "Shared-memory test fix" below.
...
- **Shared-memory test fix — Apr 26** ✅: Five `test/mem.js` tests had been failing because watr's `treeshake` opt strips imported memory when only `i32.load`/`i32.store` reference it (those ops use memory 0 implicitly without naming it). Non-shared mode survived because `(memory ...)` declaration carries an inline `(export "memory")` that anchors it. Fix at [src/compile.js:1050-1057](../src/compile.js#L1050): when `ctx.memory.shared`, also push `(export "memory" (memory 0))` to anchor the import as a treeshake root. Repro added to watr tests: 3 failing tests at [watr/test/optimize.js](../../watr/test/optimize.js) — `treeshake: keeps imported memory used by i32.load/store`, `…by memory.copy`, `…by active data segment`. Underlying watr bug remains unfixed (workaround sufficient for jz). Tests 907/912 → 912/912; bench 21/21 PASS unaffected.
...
- **Apr 26 stdlib pass — Round 2** (commits `f2bea17`, `96e9ee0`, `d64561f`): doubled down on bulk-memory + dispatch hoisting across hot string ops:
  ...
  - `__str_hash` heap branch: 4-byte unrolled FNV-1a (i32.load + 4 sequential xor/mul steps per iter).
  - `__str_copy` SSO branch: byte-loop replaced with explicit 1/2/3/4-byte stores driven off len; len≥4 lowers to single `i32.store`.
  ...
  - Bench (30×3): 21/21 PASS, raycast ~4.1ms (system thermal noise dominant: ±5%). Tests: 912/912 (after shared-memory fix). Round-2 changes are perf-neutral-to-positive on watr (compile path is dispatch-bound, not byte-throughput-bound) but materially help string-heavy synthetic loads where the bulk-copy / chunked-compare / unrolled-FNV paths dominate.

- **README updated** ✅ — extended the "What optimizations does jz apply?" table with rows for the work landed across this push: schema slot reads, pointer-type CSE, memarg fold, bulk memory ops, chunked compare, inline FNV-1a, dispatch hoisting. Refreshed perf bench numbers: typed-array sum 2.3× → 2.5×, pure scalar 50 → 41 bytes, added watr self-host row showing 21/21 native > V8 JS.

- **`hoistPtrOffset` — TESTED & REVERTED Apr 26**: Mirror of `hoistPtrType` for `__ptr_offset`. Caches offset in a local on first call, reuses on subsequent calls when source local isn't reassigned. **Unsafe**: `__ptr_offset` follows ARRAY forward chain — any intervening function call (e.g. `arr.push()`) can realloc the array and advance the chain, invalidating the cache. The "local not reassigned" check is insufficient because the heap state behind the local can mutate. Tests pass (no test exercises the realloc-during-hoist case) but bench regressed (binary size +8%, raycast worse). Comment added at [src/optimize.js:hoistPtrType](../src/optimize.js) documenting why __ptr_offset isn't hoisted.
...
- **Remaining Phase-1 levers** (Tier E below): amortized-O(1) `__arr_shift` (E1), devirtualize HANDLER closure dispatch (E5). After Apr 26 the dominant remaining cost is watr-internal compiler functions (f38 instr, f10, f179, f84) — stdlib is largely tapped out. (E3 — inline `__hash_get_local` — landed Apr 26, see below.)
- **E3 landed — Apr 26** ✅: `__dyn_get` (the hottest stdlib symbol in self-host) now inlines `__hash_get_local`'s probe loop directly. Skips a redundant type check + `i64.reinterpret_f64` + bit unboxing on already-validated `props` hash returned from `__ihash_get_local`. Refactor at [module/collection.js:400](../module/collection.js#L400). Required restructuring `__dyn_get` from `(if (result f64) ... (then UNDEF_NAN) (else inline-probe))` to imperative early-returns — the original shape tripped a watr `foldarms` bug that hoisted the common trailing `(f64.const nan:UNDEF_NAN)` out of both branches and left the inner `if` with `(result f64)` claiming a value that no longer existed. Bug fixed upstream in watr [src/optimize.js:1543](../../watr/src/optimize.js#L1543) (strip `result`/`type` annotation from rebuilt inner if when hoisting); regression test added at [watr/test/optimize.js](../../watr/test/optimize.js). Bench (PGO rebuild, 30×3): 21/21 PASS with healthy margins — containers 1.09× (was 1.07×), maze 1.07× (was 1.04×), raycast 1.05× (was 1.03×). Tests 912/912 unaffected.
...
* [ ] **Unboxed pointer ABI — SCOPING NOTE Apr 26 (milestone-scale)** (ABI change, −30–50% watr pointer traffic). Current pointers travel as 8-byte NaN-boxed f64 through every call. Replacing with a 32-bit offset + 4-bit type tag (packed into a single i32, or split dual-param `(i32 off, i32 type)`) would cut pointer traffic by half in size and remove the `__ptr_offset` / `__ptr_type` instrumentation that dominates watr's instruction count. Needs a full ABI-redesign pass: emit.js call/return lowering, closure trampoline layout, static-ptr slot layout in data segment, every stdlib. Highest-leverage but highest-risk item on the list. Overlaps with #1a/1b above — local-level unboxing already exists for narrow cases (OBJECT/SET/MAP/BUFFER); this is the cross-call version.

  **Why milestone-scale**: every stdlib helper currently signed `(param f64) (result f64)` for pointers needs a parallel `(param i32 i32) (result i32 i32)` (or packed `i32`) variant — that's ~80 helpers in module/ stdlib. Plus closure trampolines (uniform `(env, argc, a0..a7)` calling convention all-f64 today) need an i32-pointer-aware variant. Plus the FFI boundary (host imports/exports) where the JS host expects f64 NaN-box semantics — a marshalling layer is required.

  **Migration path**: incremental, type-by-type. Start with the type that has the simplest call graph and least heap-traffic — likely STRING (immutable, no forwarding). Add a parallel `__str_*_unboxed` helper family taking `(off, len)` directly. Make jz's emit choose unboxed at call sites where the static type is STRING. Measure win on a string-heavy benchmark. If positive, expand to TYPED (no forwarding either, fixed strides). ARRAY last because forwarding-on-realloc complicates the ABI (the i32 offset would need to be reloaded after any potentially-allocating call, which is harder with i32 than with f64 because f64 lets `__ptr_offset` chase the chain transparently — i32 forces every call site to re-resolve).

  **Expected gain**: 15-30% on raycast / containers / parser-heavy. Conservative upper bound is the share of cycles currently spent in `__ptr_offset` (post-E3, ~5-8% on hot paths) plus pointer-marshalling reduction (saves a `f64.reinterpret_i64` + bit-mask on every call boundary).

  **Estimated scope**: 2-3 weeks for STRING+TYPED with full coverage, 4-6 weeks for ARRAY+OBJECT+MAP+SET+CLOSURE+EXTERNAL. **Recommendation**: do this AFTER ROI work / shipping current Phase-1 wins. Phase-1 is already at 21/21 with comfortable margins post-E3; the marginal returns from squeezing further don't justify a multi-week ABI redesign until product/usage needs surface specific perf gaps that this would fill.
...
* [ ] **B1. Devirtualize closure calls when target statically known — SCOPING NOTE Apr 26.** Top remaining lever for f87 (`parseLevel`). The pattern: a const-bound closure variable like `let parseAtom = (n) => ...` later called as `parseAtom(x)`. Today this routes through `__call_closure` → `call_indirect` even though the target is statically resolvable. Fix shape: in the analysis phase, when a local is bound once to a closure expression (`let f = (...) => ...`) and never reassigned/escaped, tag the binding with a static funcref. At the call site, emit `call $closure_body_N` directly — passing the captured env explicitly when the body needs it. Closure trampolines stay for cases where the closure escapes (passed as argument, stored in object, returned). Affects analyze.js (closure-binding tracker), emit.js (call-site lowering), ir.js (env-passing convention). Scope: ~1 day. Expected gain: 5-10% on parseLevel-heavy workloads (raytrace, maze). Subsumes E5 if generalized to detect "object-of-closures" as a special case (per-key lookup → per-key direct call). Strong candidate for the next implementation push when we return to Phase-1 perf.
* [-] **B3. Extend NaN-box elision to straight-line basic blocks — DEFERRED Apr 26.** The obvious round-trip pair cancellation (`i64.reinterpret_f64 ∘ f64.reinterpret_i64` and inverse) is already in [src/optimize.js:peepholeFolds](../src/optimize.js). The remaining piece — block-level CSE for repeated `(i64.reinterpret_f64 (local.get $X))` etc. — is captured downstream by clang on the wasm2c output (trivial for clang's GVN/CSE) and reasonably by V8's wasm tier-up. Native bench shows no signal for the extension; implementation cost (dataflow + new local insertion) outweighs the speculative gain. Revisit if profile-driven evidence emerges that a specific stdlib helper has a redundant reinterpret crossing a function-call boundary that clang can't see through.
* [ ] **B4. Full LICM-on-types** (companion to #3 above). Not started.
* [ ] **B5. Devirtualize `call_indirect` on statically-known table slots — SCOPING NOTE Apr 26.** When a function reference is loaded from a known-immutable table slot (e.g. a const closure stored in a fixed slot at module init), the `call_indirect` could be replaced by a direct `call`. Today jz emits `call_indirect` uniformly because the table-slot → funcref mapping isn't tracked. Fix shape: track table-slot writes during emit, and at call_indirect sites where the slot index is statically known and the slot was written exactly once at init, replace with direct call. The post-Apr-26 wasm2c output has 61 `CALL_INDIRECT` macro uses (each = bounds + null + sig check + dispatch); even cutting half via this opt would noticeably trim raycast hot loops. Affects ctx.js (table-slot tracker), emit.js (call_indirect lowering). Scope: ~half day. Expected gain: 1-2% on raycast — narrower than B1 because the targets are mostly already-direct-call helpers, not user closures. Lower priority than B1 unless paired with it.
...
* [ ] **E1. Amortized-O(1) `__arr_shift` (head-offset) — SCOPING NOTE Apr 26.** Add `headOff` (i32) to ARRAY header at byte offset −12 (currently header is `[size:i32 @ -8, cap:i32 @ -4]`; extend to `[head:i32 @ -12, size:i32 @ -8, cap:i32 @ -4]` and update `__alloc_hdr` accordingly). `shift()` increments `headOff` instead of `memory.copy`. When `headOff > cap/4`, compact: `memory.copy(off, off + headOff*8, size*8)` and reset headOff to 0. Affects [module/array.js:537](../module/array.js#L537) (shift body) + every `__arr_get`/`__arr_set`/`__arr_len`/`__arr_*` site that indexes (must add `headOff` to the slot computation). Also affects ARRAY iteration loops in `__arr_concat`, `__arr_slice`, `__arr_map`, etc. — they need to traverse from `off + headOff*8` not `off`. **HUGE leverage on watr** — `nodes.shift()` in instr() is the dominant cost for `f38` (34.5% of raycast); 1000-instr functions do ~500K array shifts each O(n²) in the current implementation. After E1: O(1) amortized, eliminates that 500K * average-shift-cost from the critical path. Estimated win: 8-15% on raycast and similar shift-heavy paths. Scope: ~2 days because of the touch surface (every ARRAY indexing op + `__alloc_hdr`). Risk: header layout shift can introduce subtle off-by-one bugs in any code that does manual pointer arithmetic on arrays — needs careful test coverage. Highest-ROI Phase-1 lever remaining.
...
* [x] **E3. Inline `__hash_get_local` in `__dyn_get` hot path — DONE Apr 26.** `__dyn_get` is the single hottest stdlib symbol in watr self-host (~95M calls). Now inlines the probe directly after `__ihash_get_local` returns a known-HASH `props` — saves redundant type check + bit unboxing per call. Imperative early-return shape avoided a watr `foldarms` bug (fixed upstream, regression test added). 21/21 PASS with stronger margins (containers/maze/raycast +2-4 pp). Note: did NOT extend to `__hash_get_local` callsites with statically-known small HASH targets (the original idea — pre-hash known short keys at compile time). That remains as a future micro-opt; the `__dyn_get` inline captured the bulk of the 95M-call cost since it dominates over direct `__hash_get_local` use.
...
* [-] **E5. Devirtualize HANDLER closure dispatch — DEFERRED Apr 26 (scoping note).** Premise reviewed: original framing assumed call sites of the form `HANDLER[const_op]` that could be statically resolved. Survey of [watr/src/compile.js:961-971](../../watr/src/compile.js#L961-L971) shows there are no such sites — every `HANDLER[op]` and `INSTR[op]` references a runtime `op`. Devirtualization in this context means a more invasive change: recognize `IMM`/`HANDLER`-shaped object literals (const, all-closure values, never reassigned) and lower property-lookup-then-call (`HANDLER[op](...)`) to a perfect-hash switch dispatching directly to the underlying closure body, bypassing the `__dyn_get → closure-pointer → call_indirect` chain. Implementation cost: new analysis pass (closure-table promotion), new emit path (switch dispatch), schema integration so the per-key offsets are known. Estimated 2-3 days. Expected gain: 3-5% on raycast (the dispatch is already as fast as `__dyn_get` can be after E3 — the remaining cost is the `call_indirect` itself plus closure trampoline overhead). Recommendation: revisit only after Unboxed pointer ABI lands, since that ABI shift will reframe what the closure trampoline looks like and may make this opt cheaper to implement on top.
...
Hot-path stdlib pass landed Apr 26 across two rounds. Round 1 (commits `99ce0c7`, `fc50c46`), Round 2 (`f2bea17`, `96e9ee0`, `d64561f`, `ae4fa23`). PGO-confirmed wins on watr self-host.

* [x] **`__eq` bit-eq fast path** — i64 equality of NaN-box bits before falling through to typed compare. PGO: `__eq` 207M calls/run, dominant fast path now.
* [x] **`__ptr_offset` slimmed** — drop redundant `memory.size` bound check (alloc bumps already validate). PGO: 637M → 446M (Round 1) → 147M (after Round 2 dispatch hoisting).
* [x] **`__len` ARRAY fast path** — direct length-slot read before generic dispatch. PGO: 89M, mostly inline now.
* [x] **`__str_hash` heap branch — 4-byte unrolled FNV-1a** — `i32.load` + 4 sequential xor/mul steps per iter. PGO: 95M → 67M.
* [x] **`__str_eq` 4-byte chunked compare** — unaligned `i32.load` per step (~4× inner-loop throughput). PGO: 63M → 54M.
* [x] **`__mkstr` byte-loop → `memory.copy`** — lowers to memcpy under wasm2c+clang. PGO: 62M.
* [x] **Bulk-memory rewrite of `__str_slice` / `__str_pad` / `__str_repeat` / `__str_encode`** — invariant SSO/heap dispatch hoisted out of the byte loop; bulk path delegates to `__str_copy`/`memory.copy`. `__str_repeat` does doubling-via-`memory.copy` after first emit.
* [x] **Dispatch hoisting for `__str_case` / `__str_indexof` / `__str_startswith` / `__str_endswith`** — SSO/heap dispatch lifted out of inner byte loop; per-byte fetch is inline branchless extract instead of `__char_at` call. PGO: `__char_at` 137M → 91M.
* [x] **`__str_copy` SSO branch unrolled** — explicit 1/2/3/4-byte stores driven off len; len≥4 collapses to single `i32.store`.
* [x] **`stdlibDeps` swept** — `__char_at` removed from sites that no longer need it; deps narrowed to `__str_byteLen` / `__str_copy` / `__alloc` only.

**Considered but not landed (documented for future):**

* [-] **`hoistPtrOffset`** — REVERTED. Mirror of `hoistPtrType` for `__ptr_offset`. Tests passed but bench regressed (binary +8%, raycast worse). Root cause: `__ptr_offset` follows ARRAY forward chain — any intervening function call (e.g. `arr.push()`) can realloc the array and advance the chain, invalidating the cached offset. The "local not reassigned" check is insufficient because the heap state behind the local can mutate. See safety comment at [src/optimize.js](../src/optimize.js) hoistPtrType docstring.
